...
# Locators for elements related to working permit rejection
...
${REASON_FIELD}         //textarea[@class='swal2-textarea']                 # XPath locator for the text area where rejection reasons are input
${REJECT_BUTTON}        //button[@class='swal2-confirm swal2-styled']       # XPath locator for the button to reject the working permit

# Reason for rejection
${REASON}               reject reason   # Reason text to be entered during rejection

# Browser settings
${BROWSER}              Chrome  # Specifies the browser type to be used for testing (e.g., Chrome)

*** Test Cases ***
# Test case to reject a working permit by Facility Management
Working Permit Rejection By Facility Management
    [Documentation]    This test case simulates the rejection process of a working permit by a Facility Management user. The test logs into the application, navigates to the working permit section, and performs the rejection action.
    ...
    Wait Until Element Is Visible    ${REVIEW_LINK}    timeout=30 seconds  # Waits until the review link is visible to ensure the page is loaded
    Click Link    ${NEWEST_REVIEW_LINK}              # Clicks on the newest review link to access the review section

    # Step 3: Perform the review action and input the rejection reason
    ...
    Input Text    ${REASON_FIELD}    ${REASON}       # Inputs the reason for rejection into the text area
    Click Button    ${REJECT_BUTTON}                 # Clicks the reject button to confirm the rejection
    ...
    Location Should Be    ${WP_URL}                                      # Verifies that the user is redirected to the correct URL after rejection
